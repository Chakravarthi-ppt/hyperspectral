...
//        for (float v : index) {
//            int bin = static_cast<int>((v - idxMin) / (idxMax - idxMin) * kBins);
...
//    // The cluster with the higher mean brightness across feature bands is
//    // taken as "object" (built/bare structures tend to be brighter / more
//    // spectrally distinct than vegetated or open ground in this 2-cluster split).
//    double meanRow0 = cv::mean(centers.row(0))[0];
//    double meanRow1 = cv::mean(centers.row(1))[0];
//    int objectCluster = meanRow0 > meanRow1 ? 0 : 1;
