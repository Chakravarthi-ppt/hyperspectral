...
//        // Derive a scene-specific name from the zip filename so multiple
//        // different scenes can coexist in Hyperspectral_Data/ without
//        // overwriting each other.
//        QString zipName = QFileInfo(QString::fromStdString(path)).completeBaseName();
...
//    // For single-file zips, extract to Hyperspectral_Data/<zipname>/ so
//    // the result is also persistent and reusable.
//    QString zipName2 = QFileInfo(QString::fromStdString(path)).completeBaseName();
...
//    if (!proc.waitForFinished(120000)) { // 2 minute ceiling for very large archives
//        throw hsi::HsiError("Extracting '" + path + "' timed out after 2 minutes.");
//    }
