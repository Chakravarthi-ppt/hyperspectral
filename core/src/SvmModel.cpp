...
////        throw HsiError("SvmModel::train: feature/label count mismatch or empty training set.");
////    }
...
////                std::to_string(nFeatures) + " features.");
////}

////int SvmModel::predict(const std::vector<float>& feature) const {
////    cv::Mat sample(1, static_cast<int>(feature.size()), CV_32F);
////    for (size_t j = 0; j < feature.size(); ++j) sample.at<float>(0, static_cast<int>(j)) = feature[j];
...
////                batch.at<float>(col, b) = cube.at(b, row, col);
