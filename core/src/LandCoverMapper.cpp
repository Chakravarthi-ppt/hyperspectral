...
RasterCube LandCoverMapper::classifyByIndices(const RasterCube& cube,
    ...
    bool haveNdvi = bands.nir >= 0 && bands.red >= 0;
    RasterCube ndvi;
    if (haveNdvi) {
        ndvi = SpectralIndices::ndvi(cube, bands.nir, bands.red);
    ...
    if (bands.swir1 >= 0 && bands.nir >= 0) {
        auto ndbi = SpectralIndices::ndbi(cube, bands.swir1, bands.nir);
    ...
    if (bands.swir1 >= 0 && bands.red >= 0 && bands.nir >= 0 && bands.blue >= 0) {
