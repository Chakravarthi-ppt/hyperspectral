...
//    if (bandIndex < 0 || bandIndex >= cube.bands) { clear(); return; }
//    float lo, hi;
//    auto values = stretchBand(cube, bandIndex, lo, hi);

//    QImage img(cube.width, cube.height, QImage::Format_Grayscale8);
//    for (int row = 0; row < cube.height; ++row) {
//        uchar* line = img.scanLine(row);
//        for (int col = 0; col < cube.width; ++col) {
//            float v = values[row * cube.width + col];
//            float t = (v - lo) / (hi - lo);
//            t = std::clamp(t, 0.0f, 1.0f);
//            line[col] = static_cast<uchar>(t * 255.0f);
//        }
//    }
//    setImage(img);
//}
...
//    QImage img(cube.width, cube.height, QImage::Format_RGB888);
//    for (int row = 0; row < cube.height; ++row) {
//        uchar* line = img.scanLine(row);
//        for (int col = 0; col < cube.width; ++col) {
...
//            };
//            line[col * 3 + 0] = stretch(rVals[idx], rLo, rHi);
//            line[col * 3 + 1] = stretch(gVals[idx], gLo, gHi);
//            line[col * 3 + 2] = stretch(bVals[idx], bLo, bHi);
//        }
//    }
//    setImage(img);
//}
...
//    const QColor fallback(40, 40, 40); // unexpected/out-of-range label -> visually obvious, not silently miscolored
...
//        uchar* line = img.scanLine(row);
//        for (int col = 0; col < cube.width; ++col) {
//            int label = static_cast<int>(cube.data[base + static_cast<size_t>(row) * cube.width + col]);
//            auto it = styles.find(label);
//            const QColor& c = (it != styles.end()) ? it->second.color : fallback;
//            line[col * 3 + 0] = static_cast<uchar>(c.red());
//            line[col * 3 + 1] = static_cast<uchar>(c.green());
//            line[col * 3 + 2] = static_cast<uchar>(c.blue());
//        }
//    }
//    setImage(img);
//    setLegend(styles);
